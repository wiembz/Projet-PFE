...
KEY_COLUMNS = ("NUMCNT", "NUMAVT", "NUMMAJ")
DATE_COLUMNS = ("DEBCNT", "FINCNT", "DEBEFFET", "FINEFFET", "DATEPREC")
NUMERIC_COLUMNS = ("TOTAL_PRIME",)
INTEGER_COLUMNS = ("DUREE",)
...
def main() -> None:
    args = _parse_args()
    stats = clean_production(args.etl_run_id)
...
def clean_production(etl_run_id: str) -> CleanStats:
    _validate_etl_run_id(etl_run_id)

    input_file = BRONZE_DIR / etl_run_id / "production.parquet"
    output_file = SILVER_DIR / etl_run_id / "production.parquet"

    if not input_file.exists():
        raise FileNotFoundError(f"Bronze production file not found: {input_file}")

    dataframe = pd.read_parquet(input_file)
    input_count = len(dataframe)

    _assert_required_columns(dataframe)
    cleaned = _clean_production_dataframe(dataframe)

    null_key_mask = cleaned["contrat_business_key"].isna()
    null_key_count = int(null_key_mask.sum())
    keyed = cleaned.loc[~null_key_mask].copy()

    deduplicated = _deduplicate_production(keyed)
...
def _clean_production_dataframe(dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    _convert_integer_columns(cleaned)
    _add_contrat_business_key(cleaned)
...
def _convert_integer_columns(dataframe: pd.DataFrame) -> None:
    for column in INTEGER_COLUMNS:
        if column in dataframe.columns:
            dataframe[column] = parse_integer(dataframe[column])


def _add_contrat_business_key(dataframe: pd.DataFrame) -> None:
    key_mask = (
        dataframe["NUMCNT"].notna()
        & dataframe["NUMAVT"].notna()
        & dataframe["NUMMAJ"].notna()
    )
    business_key = pd.Series(pd.NA, index=dataframe.index, dtype="string")
    business_key.loc[key_mask] = (
        dataframe.loc[key_mask, "NUMCNT"].astype("string")
        + "|"
        + dataframe.loc[key_mask, "NUMAVT"].astype("string")
        + "|"
        + dataframe.loc[key_mask, "NUMMAJ"].astype("string")
    )

    insert_at = dataframe.columns.get_loc("NUMMAJ") + 1
    dataframe.insert(insert_at, "contrat_business_key", business_key)


def _deduplicate_production(dataframe: pd.DataFrame) -> pd.DataFrame:
    if dataframe.empty:
    ...
    sort_columns = ["contrat_business_key", "_completeness_score", "_original_order"]
...
def _assert_required_columns(dataframe: pd.DataFrame) -> None:
    ...
    if missing_metadata:
        ...
        raise ValueError(f"Bronze production file is missing metadata columns: {missing}")

    missing_keys = [column for column in KEY_COLUMNS if column not in dataframe.columns]
    if missing_keys:
        missing = ", ".join(missing_keys)
        raise ValueError(f"Bronze production file is missing key columns: {missing}")
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Clean Bronze production parquet into Silver production parquet."
    )
    parser.add_argument(
        "--etl-run-id",
        required=True,
        help="ETL run id containing data/bronze/<etl_run_id>/production.parquet",
    )
