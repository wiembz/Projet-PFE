...
TARGET_TABLE = "dim_delegation"
SOURCE_NATURAL_KEY = "delegation_natural_key"
SOURCE_COLUMN = "IDDELEGA"
SOURCE_SYSTEM_VALUE = "Production.xlsx/Sinistres.xlsx"
TARGET_KEY_CANDIDATES = (
    "delegation_natural_key",
    "delegation_business_key",
)
NEVER_INSERT_COLUMNS = {"delegation_sk", "created_at", "updated_at"}
...
SOURCE_TO_TARGET_CANDIDATES = {
    ...
    SOURCE_COLUMN: ("code_delegation", "id_delegation", "iddelega"),
...
def main() -> None:
    args = _parse_args()
    stats = load_dim_delegation(args.etl_run_id)
    print(f"input_count={stats.input_count}")
    print(f"distinct_delegation_count={stats.distinct_delegation_count}")
...
def load_dim_delegation(etl_run_id: str) -> LoadStats:
    _validate_etl_run_id(etl_run_id)

    dataframe = _read_inputs(etl_run_id)
    input_count = len(dataframe)
    delegation_rows, null_key_count = _build_distinct_delegation_rows(dataframe)
    ...
    try:
        with conn.cursor() as cur:
            ...
            rows = _build_load_rows(delegation_rows, column_mapping)
            loaded_count = _upsert_dim_delegation_rows(
                cur,
                rows,
                key_column,
                table_columns,
    ...
    return LoadStats(
        ...
        distinct_delegation_count=len(delegation_rows),
...
def _read_inputs(etl_run_id: str) -> pd.DataFrame:
    ...
    for source_name, file_name in INPUT_FILES:
        ...
        if SOURCE_COLUMN not in frame.columns:
            raise RuntimeError(f"Silver {source_name} file is missing {SOURCE_COLUMN}")

        frames.append(frame[[SOURCE_COLUMN]].copy())

    if not frames:
        missing = "\n".join(str(path) for path in missing_files)
        raise FileNotFoundError(f"Silver delegation input files not found:\n{missing}")

    return pd.concat(frames, ignore_index=True)


def _build_distinct_delegation_rows(dataframe: pd.DataFrame) -> tuple[pd.DataFrame, int]:
    delegations = dataframe.copy()
    delegations[SOURCE_COLUMN] = delegations[SOURCE_COLUMN].astype("string").str.strip()
    delegations[SOURCE_COLUMN] = delegations[SOURCE_COLUMN].mask(
        delegations[SOURCE_COLUMN].str.lower().isin(NULL_MARKERS),
        pd.NA,
    )

    null_key_count = int(delegations[SOURCE_COLUMN].isna().sum())
    delegations = delegations.loc[delegations[SOURCE_COLUMN].notna()].copy()

    delegations[SOURCE_NATURAL_KEY] = delegations[SOURCE_COLUMN]
    delegations["source_system"] = SOURCE_SYSTEM_VALUE

    distinct = (
        delegations.sort_values(SOURCE_NATURAL_KEY, kind="mergesort")
...
def _upsert_dim_delegation_rows(
    cur: Any,
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Load Silver delegation values into iris_dw.dim_delegation."
    )
    parser.add_argument(
        "--etl-run-id",
        required=True,
